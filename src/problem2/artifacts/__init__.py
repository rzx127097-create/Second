...
def build_artifacts(input_jsonl: Path, output_root: Path, *, manifest: Path) -> ArtifactBundle:
    ...
    summary_path.write_text(json.dumps({"provisional": True, "rows": rows, "paired_differences": paired}, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    write_evidence_manifest(Path(manifest), Path(input_jsonl), outputs, records)
