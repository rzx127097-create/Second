...
def _provenance(contract, condition: str) -> dict[str, str]:
    ...
    source = hashlib.sha256("".join(contract.file_hashes.values()).encode("ascii")).hexdigest()
    config = contract.file_hashes.get("configs/problem2/g5/methods.yaml", source)
    protocol = contract.file_hashes.get("docs/evidence/g5/heterogeneous_interface.yaml", source)
    ancestry = _hash_text(condition, 64)
    return {"source_commit": commit if len(commit) == 40 else "0" * 40, "source_bundle_sha256": source, "config_hash": config, "protocol_hash": protocol, "ancestry_hash": ancestry}
...
def _envelope(algorithm, current: dict[str, np.ndarray], nxt: dict[str, np.ndarray], index: int, details: Mapping[str, Any]):
    ...
    role_batch = RoleBatch.from_action_result(
        ...
        transition_id=f"development-10000:{index}",
    )
    common = {
...
def _write_manifest(root: Path, files: list[Path], result: Mapping[str, Any]) -> Path:
    manifest = {"schema_version": "g5-smoke-artifact-v1", "status": "pass", "maturity": "M2", "method": result["method"], "algorithm_id": result["algorithm_id"], "condition_id": result["condition_id"], "validation_accessed": False, "sealed_accessed": False, "battery_replenishment_enabled": False, "artifacts": [{"path": str(path.relative_to(root)), "sha256": artifact_sha256(path), "bytes": path.stat().st_size} for path in files]}
...
def run_training_job(job: Mapping[str, Any], device: str, max_interactions: int, output_root: Path | str) -> dict[str, Any]:
    ...
    random.seed(int(job.get("training_seed", 51001)))
    np.random.seed(int(job.get("training_seed", 51001)))
    torch.manual_seed(int(job.get("training_seed", 51001)))
    if str(device).lower() == "cuda":
        torch.cuda.reset_peak_memory_stats()
    output = Path(output_root).resolve() / f"{method}__{condition}__{int(job.get('training_seed', 51001))}"
    ...
    if interrupted:
        return {"method": method, "algorithm_id": actual_method, "condition_id": condition, "interactions": stop_at, "updates": 0, "interrupted": True, "checkpoint": str(checkpoint), "validation_accessed": False, "sealed_accessed": False, "resume_equivalent": False, "peak_allocated_bytes": int(torch.cuda.max_memory_allocated()) if str(device).lower() == "cuda" else 0, "peak_reserved_bytes": int(torch.cuda.max_memory_reserved()) if str(device).lower() == "cuda" else 0}
    ...
    resume_equivalent = bool(resume_from) and bool(finite) and bool(evaluation_frozen) and int(algorithm.diagnostics.snapshot().get("updates", 0)) >= 1
    summary = {"method": method, "algorithm_id": actual_method, "condition_id": condition, "interactions": target, "updates": int(algorithm.diagnostics.snapshot().get("updates", 1)), "finite_metrics": bool(finite), "evaluation_frozen": bool(evaluation_frozen), "evaluation_actions": evaluation_actions, "validation_accessed": False, "sealed_accessed": False, "replenished_resource": "pesticide", "battery_replenishment_enabled": False, "interrupted": False, "resume_equivalent": resume_equivalent, "resume_comparison": {"metrics_finite": bool(finite), "evaluation_actions_stable": bool(evaluation_frozen), "diagnostics_updates": int(algorithm.diagnostics.snapshot().get("updates", 0))}, "checkpoint": str(checkpoint), "provenance": provenance, "metrics": dict(metrics), "role_shapes": {"uav": [2, 179], "vehicle": [1, 28]}, "mask_shapes": {"uav": [2, 6], "vehicle": [1, 5]}, "peak_allocated_bytes": int(torch.cuda.max_memory_allocated()) if str(device).lower() == "cuda" else 0, "peak_reserved_bytes": int(torch.cuda.max_memory_reserved()) if str(device).lower() == "cuda" else 0}
